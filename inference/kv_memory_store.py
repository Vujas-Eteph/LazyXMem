...
class KeyValueMemoryStore:
    ...
    def add(self, key, value, shrinkage, selection, objects: List[int]):
        new_count = torch.zeros((key.shape[0], 1, key.shape[2]),
                                device=key.device, dtype=torch.float32)
        new_life = torch.zeros((key.shape[0], 1, key.shape[2]),
                               device=key.device, dtype=torch.float32) + 1e-7
        ...
        if self.k is None:
            ...
            # Updating the shrinkage and selection with the new memory elements
            if shrinkage is not None:
                self.s = torch.cat([self.s, shrinkage], -1)
            if selection is not None:
                self.e = torch.cat([self.e, selection], -1)
            if self.count_usage:  # used in the LFU (Least Frequently used)
        ...
        if objects is not None:
            ...
            # "I think that different groups have different memory lenghts, so might use
            # that as a way to update the memroy only for one object and not the second one?"
            for gi, group in enumerate(self.obj_groups):
                ...
                # For each group, concant the values togthere.
                self.v[gi] = torch.cat([self.v[gi], value[group]], -1)  # [n, C_v, T*H*W]
