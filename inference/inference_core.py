...
class InferenceCore:
    ...
    def get_logits(self):
        return unpad(self.pred_logits, self.pad) if self.pred_logits is not None else None
    ...
    def step(self, image, mask=None, valid_labels=None, end=False):
        ...
        if need_segment:
            ...
            hidden, self.pred_logits, pred_prob_with_bg = self.network.segment(multi_scale_features, memory_readout,
    ...
    def solo_predict(self, image, mask=None, valid_labels=None):
        '''
        Make only a prediction, don't save anything in the memory
        '''
        # image: 3*H*W
        # mask: num_objects*H*W or None
        image, self.pad = pad_divide_by(image, 16)
        ...
        is_mem_frame = False
        need_segment = True # len(self.all_labels) != len(valid_labels) # TODO Try with this one
    ...
    def solo_update(self, image, mask=None, valid_labels=None, end=False):
        # image: 3*H*W
        # mask: num_objects*H*W or None
        self.curr_ti += 1
        image, self.pad = pad_divide_by(image, 16)
    ...
    def solo_predict_EDE(self, image, mask=None, valid_labels=None, end=False):
        # image: 3*H*W
        # mask: num_objects*H*W or None
        image, self.pad = pad_divide_by(image, 16)
        image = image.unsqueeze(0) # add the batch dimension
        ...
        return unpad(pred_prob_with_bg, self.pad), pred_prob_with_bg

    def solo_update_EDE(self, image, pred_prob_with_bg_INPUT, valid_labels=None, end=False):
        # image: 3*H*W
        # mask: num_objects*H*W or None
        self.curr_ti += 1
        image, self.pad = pad_divide_by(image, 16)
        ...
        # use the input mask if any
        if pred_prob_with_bg_INPUT is not None:
            if pred_prob_no_bg is not None:
                # if we have a predicted mask, we work on it
                # make pred_prob_no_bg consistent with the input mask
                # shift by 1 because mask/pred_prob_no_bg do not contain background
        ...
        if is_mem_frame:
            value, hidden = self.network.encode_value(image, f16, self.memory.get_hidden(),
                                    pred_prob_with_bg_INPUT[1:].unsqueeze(0), is_deep_update=is_deep_update)
            self.memory.add_memory(key, shrinkage, value, self.all_labels,
                                    selection=selection if self.enable_long_term else None)
            self.last_mem_ti = self.curr_ti
