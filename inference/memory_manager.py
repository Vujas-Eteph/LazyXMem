...
class MemoryManager:
    ...
    def __init__(self, config):
        ...
        self.user_mem = KeyValueMemoryStore(count_usage=self.enable_long_term)
    ...
    def match_memory(self, query_key, selection):
        ...
        if self.enable_long_term and self.long_mem.engaged():
            ...
            memory_key = torch.cat([self.long_mem.key,
                                    self.work_mem.key], -1)
            shrinkage = torch.cat([self.long_mem.shrinkage,
                                   self.work_mem.shrinkage], -1)
    ...
    def add_memory(self, key, shrinkage, value, objects, selection=None):
        ...
        key = key.flatten(start_dim=2)              # [1, C_k, H*W]
        shrinkage = shrinkage.flatten(start_dim=2)  # [1, 1, H*W]
        value = value[0].flatten(start_dim=2)       # [n, C_v, H*W]
        ...
        # objects [i+1 for i in range(n)], contains the objects indices
    ...
    def create_hidden_state(self, n, sample_key):
        ...
        if self.hidden is None:
            ...
            # If a new object is introduced, at a new hidden state for it.
