...
class XMemTrainer:
    def __init__(self, config, logger=None, save_path=None, local_rank=0, world_size=1):
        ...
        try:
            ...
            self.XMem = XMem(config).to(device)
    ...
    def do_pass(self, data, it=0):
        ...
        with torch.cuda.amp.autocast(enabled=self.config['amp']):
            ...
            filler_one = torch.zeros(1, dtype=torch.int64).to(key.device)
            hidden = torch.zeros((b, num_objects, self.config['hidden_dim'], *key.shape[-2:])).to(key.device)
    ...
    def save_checkpoint(self, it):
        ...
        try:
            output= self.XMem.module.state_dict(),
        except AttributeError:
            output= self.XMem.state_dict(),
