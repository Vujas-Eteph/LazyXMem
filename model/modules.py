...
class FeatureFusionBlock(nn.Module):
    def __init__(self, x_in_dim, g_in_dim, g_mid_dim, g_out_dim):
        ...
        # Check the ECCV 2018 paper: 
        # https://openaccess.thecvf.com/content_ECCV_2018/papers/Sanghyun_Woo_Convolutional_Block_Attention_ECCV_2018_paper.pdf
        self.attention = CBAM(g_mid_dim)  # Is used since the STCN paper... Check again, but not usefull from the authors point of view 
        self.block2 = GroupResBlock(g_mid_dim, g_out_dim)

    def forward(self, x, g):
        batch_size, num_objects = g.shape[:2]

        # Concatenate the image features f16, with g, and expand f16 by the 
        # number of objects
        g = self.distributor(x, g)
        g = self.block1(g)
        # Apply some form of attention
...
class ValueEncoder(nn.Module):
    ...
    def forward(self, image, image_feat_f16, h, masks, others, is_deep_update=True):
        ...
        if not self.single_object:
            g = torch.stack([masks, others], 2)  # stack along the H dimension
        else:
            g = masks.unsqueeze(2)
        # Concatenate image and x, and create as many images as the number of objects
        g = self.distributor(image, g)

        batch_size, num_objects = g.shape[:2]
        # Flatten g along the batch and object dimension
        # Before flattening: g.shape = (4, 3, 2, 2) (just as an example)
        # After the flatten operation, the shape would be:
        # After flattening: g.shape = (12, 2, 2) (because 4 * 3 = 12)
        ...
        # Reshape to batch. nbr of objects, h_strided, w_strided
        g = g.view(batch_size, num_objects, *g.shape[1:])
        
        # Fuses the features from f16 with g
...
class Decoder(nn.Module):
    ...
    def forward(self, f16, f8, f4, hidden_state, memory_readout, h_out=True):
        ...
        # TODO: Check if this of shape is b,n where obects are separated, look at it, ans see if the sift aggregate can be applied with a wrapper
