...
class GConv2D(nn.Conv2d):
    '''
    Custom convolutional layer to work with multiple objects along the batches
    '''
    def forward(self, g):
        batch_size, num_objects = g.shape[:2]
...
class MainToGroupDistributor(nn.Module):
    '''
    Expand the x features to all objects.
    '''
    ...
    def forward(self, x, g):
        ...
        # Generate as many image as the number of objects along the object dim
        x_feature_for_each_obx = x.unsqueeze(1).expand(-1, num_objects,
                                                       -1, -1, -1)
        if self.method == 'cat':
            if self.reverse_order:
                g = torch.cat([g, x_feature_for_each_obx], 2)
            else:
                g = torch.cat([x_feature_for_each_obx, g], 2)
        elif self.method == 'add':
            g = x_feature_for_each_obx + g
        else:
